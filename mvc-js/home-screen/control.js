function Controller(model, view) {
    ...
    var setupEventListeners = function () {
        ...
        let catagoryId = [DOM.Neuropathology, DOM.EarTherapy, DOM.EyesightTherapy, DOM.HeartTherapy, DOM.GastroEnterology]
        
        document.querySelector(DOM.submitBtn).addEventListener('click', submitFunction , false)

        for (let index in catagoryId)
            document.querySelector(catagoryId[index]).addEventListener('click', f=>{
    ...
    return {
        ...
        init: function () {
            ...
            view.startUpEvent(model.allTotalAppointments())
            setupEventListeners()
        }
