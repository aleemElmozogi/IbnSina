function View() {
    ...
    return {
        ...
        whelcomeMessage: ()=>document.getElementById(DOMstrings.doctorName).innerHTML ="Doctor " + pdName +" profile",

        reports: function(){
            document.getElementById(DOMstrings.totalAppointments).innerHTML = allTotalAppointments()
            document.getElementById(DOMstrings.myAppointments).innerHTML = totalAppointments()
            document.getElementById(DOMstrings.femalePations).innerHTML = femalePations()
        ...
        pTable: function(appointmentsGetter) {
            ...
            for(let index in doctorAppointmentsGetter(findDoctor()) ){
              ...
              tdappointmentDate.innerHTML = appointmentsGetter()[index].appointmentDate
        ...
        sideBarDoctroDetails: function(doctorsGetter, findDoctor) {
            document.getElementById(DOMstrings.sdName).innerHTML = doctorsGetter()[findDoctor()].dName
            document.getElementById(DOMstrings.ssCost).innerHTML = doctorsGetter()[findDoctor()].sCost
            document.getElementById(DOMstrings.sdRate).innerHTML = doctorsGetter()[findDoctor()].dRate
            document.getElementById(DOMstrings.sdDegree).innerHTML = doctorsGetter()[findDoctor()].dDegree
                    },
        ...
        getDoctorName:()=>  document.querySelector(DOMstrings.chooseDoctor).value.trim(),
        ...
        startUpEvent: function(appointmentsGetter, doctorsGetter, findDoctor, doctorName){
            todayDate()
            whelcomeMessage()
            reports()
            pTable(appointmentsGetter)
            sideBarDoctroDetails(doctorsGetter, findDoctor)
            pChoseDoctor(doctorName)
